...
\item Stage 2: Find and parse each cross reference table or incremental update. 
  ...
  information) in order to know \emph{where} and \emph{what} to process.
...
\item Stage 4: Transform the XRef map to an object map. This stage is complex and
  ...
  If all goes well, we have a syntactically correct candidate DOM.
\item Stage 5: Validate candidate DOM.  This stage takes the candidate DOM and
  semantically verifies that it represents a sensible document graph per the PDF Standard.
  E.g., each object in the DOM is well-formed with all required data types and values, 
  no unexpected recursion, etc.
\item Stage 6: Render the validated DOM. Here we render the validated DOM, or parts thereof,
...
of such a model for code generation, DOM validation, or test case generation can reduce the tediousness.
