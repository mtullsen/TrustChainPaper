...
In this section we define \lstcd{pPDFDom} which does the parsing \&
...
Line 1 is \lstcd{pPDFDom}'s type signature, indicating 
that it is a ``monadic parser'' \lstcd{P} that returns a
value of type \lstcd{DOM}.

Monads don't need to be complex, this is what you need to know
for this paper:
\begin{itemize}
  ...
  \item Since Haskell is purely functional, we have a simple monad
    \lstcd{P} that captures, under the hood, our desired effects.
  \item Our \lstcd{P} monad supports
    \begin{itemize}
...
free, or update objects.  Note \lstcd{XRefEntry}.
