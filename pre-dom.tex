...
\item The standard primarily defines just one thing: \emph{what is a valid PDF file},
  ...
  (5) Behavior when given an incorrect PDF.
\end{itemize}
...
For simplicity, the definition does not constrain \objids{} using the values bound to trailer dictionaries' \lstcd{Size} key as required in the standard, specifically that:
\begin{itemize}
  ...
  \item The value shall be greater by one than the highest object number defined in the PDF file.
\end{itemize}
...
\item There are many \lstcd{validate} instances we might add,
  ...
  \item validate that the offset of new objects in the update are
    defined in the ``body region'' of the respective update (i.e., not point
