...
Ideally, every PDF implementation should:
\begin{itemize}
\item comply with the PDF standard,
\item when not fully supporting the standard, do so gracefully and inform the user,
...
\item The intrinsic complexity of PDF:
  the PDF file format is a less than ideal design that reflects almost 30 years of
  an evolving standard.
  %
  PDF has multiple redundant and deprecated features and is explicitly designed to facilitate efficient processing of
  large files (``efficiency hacks'').
  %
  PDF involves multiple sublanguages, numerous embedded formats, and
  contains complex parsing needs, such as backwards searching and
...
\item Lack of formality in the standard. Thus, implementators
  ...
  Because writing a PDF implementation from scratch is a significant and costly effort,
...
\item The standard primarily defines just one thing: \emph{what is a valid PDF file},
  with only a few processor requirements. Many technical details are also delegated to the 
  numerous normative references which leaves the majority of decisions up to each implementation:
  (1) What should absolutely \emph{not} be allowed (because in the real world
    ...
    what are deemed to be acceptable or reasonable error recovery methods?
  (2) What is done when redundant features are inconsistent with one
    another: which, or neither, has priority?
  (3) What is \emph{required} from the PDF writer versus
    what do we \emph{require} the PDF reader to check in every file?
  (4) Behavior when given an incorrect PDF: is the behavior undefined or is
      the implementation required to catch this?
\end{itemize}

Failure to faithfully implement the standard can result in 
parser differentials as well as direct vulnerabilities (such as
...
the standard, understanding the vulnerabilities, and aiding implementors
of PDF.
...
functional programming language with a lazy; \todo{MT - incomplete sentence?}
%
...
parsers (parsing integers, parsing cross reference entries, etc.) and the
...
%
The full specification of DaeDaLus is publicly available~\cite{daedalusrepo}.
...
This specification supports the latest PDF 2.0, including compressed objects and cross-reference streams.
%
...
For simplicity, the definition does not constrain \objids{} using the values bound to trailer dictionaries' \lstcd{Size} key as required in the standard.
%
...
%
Also, the value shall be greater by one than the highest object number defined in the PDF file.
...
\item There appear to be multiple ways to enforce \lstcd{< Size} as we
  process individual updates. \todo{didn't understand what needs to be enforced here}
\item Updates need not be processed if they occur before the definition of all objects (which can be determined from 
  \lstcd{Size}).
\item \lstcd{validate} instances could prohibit double frees of objects, updates of unfree objects, updates that do not increment the generation number, frees of non-existent object ID's, and other unconventional uses of generation numbers.
%
Other instances could validate that the offset of new objects in each update are defined in update's ``body region'' (i.e., they do not point to previous nor subsequent regions).
\end{itemize}
%
None of the above properties can be validated until the completion of Stage 3.
%
...
%
In stage 4.1, traditional \lstcd{XRef}'s are resolved and the resulting objects are parsed; see line 3 \todo{what line does this reference?}:
\begin{code}
