...
The key things to note are
 - \lstcd{XREF_ENTRY} and \lstcd{DOM_ENTRY} are both types that mutate progressively
   from unparsed/unknown to fully evaluated.
 - \lstcd{deref()} and \lstcd{parseObject()} are mutually recursive functions.
 - it could be complicated to implement \lstcd{infiniteLoopDetected()}.
 - this implementation is naturally ``lax'': it would for instance
   allow a \lstcd{Length} to be stored in an \lstcd{ObjStm} stream as
