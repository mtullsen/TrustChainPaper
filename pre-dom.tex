...
This section
introduces our specification language (\Cref{sec:spec-language}),
presents our specification's core definitions (\Cref{sec:core}),
followed by its definitions of each
stage of the PDF Trust Chain (\Crefrange{sec:stage-1}{sec:stage-4}).

\subsection{Specifying PDF: The Language, Haskell}
...
relevant.
% benefits:
In addition to the above general benefits of formal definitions, Haskell is
...
we view the current definition as a significant first step toward a
definition that is both clear and executable.

% say more about the spec's scope:
The specification formalizes Pre-DOM processing, stages 1 to 4
(\Cref{fig:pdf-trust-chain}).
%
It relies on having primitive parsers (whose definitions we do not include).
Other formalisms, such as the publicly available \emph{DaeDaLus Data Description
  Language}~\cite{daedalusrepo}, are better suited to specifying these.
%
...
construct that allows us to to sequence effectful \emph{actions}
