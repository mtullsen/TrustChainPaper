...
that happens after the DOM is created (stage 5 \& 6).
%
...
{\bf{Stage 2: we find and parse any incremental updates}}
...
{\bf{Stage 3: ... parse xref table and ...}}
  
Stage 3: data in each cross reference table must then be parsed to
...
Stage 4: using information from Stages 1, 2 and 3, the final set of
...
The \lstcd{pDOM} function definition is now finished.
In the next two sections we will break down these functions
