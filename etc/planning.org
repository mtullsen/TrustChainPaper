...
combineUpdates seek updates =
    do
    let (xref,dict):us = updates -- safe: updates never null
    xrefEntries <- mapM (thawXRefEntry xref) (getObjIds xref)

    let tbl0 = M.fromList xRefEntries
    tblR  <- foldlM mergeUpdate tbl0 us
    return (removeFrees tblR)

mergeUpdate :: (Map ObjId XRefEntry)
            -> (XRefRaw, TrailerDict)
            -> P (Map ObjId XRefEntry)
mergeUpdate map0 (xref,dict) =
    do
    let ois = getObjIds xref
    newEntries <- forM (ois \\ M.keys map0) $
                    \oi-> do
                          entry <- thawXRefEntry xref oi
                          return (oi,entry)
    return (M.union map0 (M.toList newEntries))
...
thawXRefEntry :: RawXRef -> ObjId -> P XRefEntry
thawXRefEntry = stub
...
#+end_src    
  ...
  - really just want to thaw needed ObjId's
